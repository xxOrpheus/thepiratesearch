...
app.get('/search', async function(req, resp) {
	if(req.query.s) {
		...
		await getJson(url).then(function(json) {
			resp.render('search', { title: 'search results', moment: require('moment'), results: json, getTrackers: getTrackers, getMagnet: getMagnet, getCategory: getCategory, search: req.query.s, category: category});
		});
	}
});

app.get('/', function(req, resp) {
	resp.render('index', {title: 'Search ThePirateBay'});
});
